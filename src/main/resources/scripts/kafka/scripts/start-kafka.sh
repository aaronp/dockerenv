...
if [ ! -z "$ADVERTISED_HOST" ]; then
    ...
    if grep -q "^advertised.host.name" $KAFKA_HOME/config/server.properties; then
        echo "replacing advertised host"
        sed -r -i "s/#(advertised.host.name)=(.*)/\1=$ADVERTISED_HOST/g" $KAFKA_HOME/config/server.properties
    else
        echo "setting advertised host"
        echo "\n\nadvertised.host.name=$ADVERTISED_HOST\n" >> $KAFKA_HOME/config/server.properties
    fi
else
    echo "ignoring ADVERTISED_HOST: $ADVERTISED_HOST"
fi

if [ ! -z "$ADVERTISED_PORT" ]; then
    echo "advertised port: $ADVERTISED_PORT"
    if grep -q "^advertised.port" $KAFKA_HOME/config/server.properties; then
        echo "replacing advertised port"
        sed -r -i "s/#(advertised.port)=(.*)/\1=$ADVERTISED_PORT/g" $KAFKA_HOME/config/server.properties
    else
        echo "setting advertised port"
        echo "\nadvertised.port=$ADVERTISED_PORT\n" >> $KAFKA_HOME/config/server.properties
    fi
else
    echo "ignoring ADVERTISED_PORT: $ADVERTISED_PORT"
fi

if grep -q "^listeners" $KAFKA_HOME/config/server.properties; then
    echo "replacing listeners"
    #sed -r -i "s/#(listeners)=(.*)/\1=PLAINTEXT://$ADVERTISED_HOST:8083/g" $KAFKA_HOME/config/server.properties
else
    echo "setting listeners"
    #echo "\nlisteners=PLAINTEXT://$ADVERTISED_HOST:8083\n" >> $KAFKA_HOME/config/server.properties
fi
